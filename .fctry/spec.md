...
- **@setlist/mcp** -- The MCP server. A thin translation layer wrapping @setlist/core as 56 MCP tools via @modelcontextprotocol/sdk, using stdio transport managed by Claude Code's lifecycle.
...
The four packages share the same SQLite database (schema v15 as of spec 0.29) at `~/.local/share/project-registry/registry.db`. Library consumers (Chorus, Ensemble) import `@setlist/core` directly rather than opening the file.
...
- Behavioral correctness is carried by the 168-scenario holdout set in `.fctry/scenarios.md` (see §4.5 `#testing-discipline`), evaluated by LLM-as-judge; vitest is available for targeted unit tests against @setlist/core where a fast, local signal is useful.
...
**Bootstrap primitives are not capabilities.** The user-composable bootstrap primitive system introduced in v0.28 (see `#project-bootstrap` (2.13) and `#desktop-app` (2.14)) is structurally distinct from capability declarations. Primitives are an internal data type — recipe steps the bootstrap engine knows how to execute — not a re-declared integration surface. They do not flow through `register_capabilities`, do not appear in `query_capabilities`, and do not extend the library/cli/mcp self-registration triad described below. A user-authored `mcp-tool` primitive that delegates to, say, a Todoist MCP server's `create_project` tool is a recipe step pointing at someone else's capability — it does not register a new setlist capability and does not change setlist's own capability set. The triad self-registration contract is unchanged.
...
- **MCP tools** — every tool the server exposes (currently 56), registered with `type: tool`. The source of truth is the tool-registration array in the MCP server entrypoint; whatever the server is about to expose to clients is what lands in the registry.
...
The registry can create a new project end-to-end: registering its identity, scaffolding its folder, populating it from templates, optionally initializing version control, optionally creating a Mail.app mailbox for the project's correspondence, and optionally running additional steps the user has composed onto the project's type. Bootstrap is a **recipe runner**: each project type carries an ordered list of bootstrap **primitives** with bound parameters, and the engine walks that recipe in order. This subsumes the manual `new-project.sh` shell script entirely — bootstrap is a first-class registry operation available through MCP, CLI, and library import. Behavioral contract: scenarios S139–S168.

**Three primitive shapes.** A primitive is one named, parameterized step the engine knows how to execute. Three shapes are supported in v1; the set is closed (no plugin-style code primitives, no `http-call` shape):

- **`filesystem-op`** — Creates folders, copies files or directories, appends to text files. Parameters identify paths and content; the engine resolves `{project.name}`, `{project.path}`, `{project.type}`, `{project.parent_path}`, `{project.type.template_directory}`, and `{project.email_account}` tokens at run time (see "Template tokens" below for the full set and resolver behavior). The three filesystem-touching built-ins (create-folder, copy-template, update-parent-gitignore — see below) are filesystem-op primitives.
- **`shell-command`** — Runs a verbatim string under the user's shell, in the new project's folder. The string may be multi-line. Only `{project.*}` tokens are substituted; nothing else is rewritten. Exit code zero is success; non-zero is failure. The user's shell environment is inherited as-is (PATH, keychain, `gh` auth, `op` session, AWS profile, etc.). The git-init built-in is the canonical shell-command primitive.
- **`mcp-tool`** — Delegates to an MCP tool registered with the host MCP client (Claude Code, Claude Desktop, or any other client running the bootstrap session). The primitive carries the tool name and a parameter map; the engine forwards the call through the host's tool surface and observes the response. Auth, networking, and rate limits live in the MCP server itself — setlist never touches credentials. If the named tool is not currently registered with the session, pre-flight fails before the recipe runs.

The closed shape set is a safety boundary, not a temporary scaffold: it is what makes "no arbitrary code execution" observable. See `#hard-constraints` (4.3).

**Built-in primitives.** Five seeded built-in primitives ship as read-only, user-droppable entries; one engine-only structural step rounds out the surface:

- **create-folder, copy-template, git-init, update-parent-gitignore** — these four refactor into the registry as **read-only, user-droppable built-in primitives**. They appear in the Primitives panel in Settings (see `#desktop-app` (2.14)) and may be dropped onto any project type's recipe in any order. They cannot be edited or deleted. Their parameters bind to project-type fields where natural (e.g., copy-template's source binds to the type's template directory).
- **mail-create-mailbox** (added in spec 0.29) — a `shell-command` shape primitive that creates a Mail.app mailbox under a specified account at a templated path. It composes from the existing closed three-shape set; **no new primitive shape is introduced**. The primitive runs `osascript` to invoke AppleScript's `make new mailbox` against the running Mail.app process; AppleScript is silently idempotent when a mailbox of the same name already exists under the same account, so duplicate-name re-runs succeed without creating a sibling or raising an error. Parameters: an `account` template (typically bound to `{project.email_account}`) and a `mailbox_name` template (default `Projects/{project.name}`; `/` separators produce nested mailboxes per Mail.app's convention). **Pre-flight checks only that Mail.app is running** via a non-side-effecting process probe — account existence is NOT validated at pre-flight (account enumeration on every bootstrap is heavier than process detection, and the cost-to-correctness tradeoff favors the lighter check). If Mail.app is not running, pre-flight fails cleanly with "Mail.app must be running to create a mailbox." Setlist does **not** auto-launch Mail.app — that would be a silent side effect and is explicitly forbidden. If the resolved account does not exist on the running Mail.app, the AppleScript fails at execution and surfaces through the standard Retry/Skip/Abandon dialog — same path as any other shell-command failure. The primitive is **seeded as a built-in and visible in the Settings primitive picker, but is NOT included in any project type's default recipe**. Users opt in by adding the step to a project type's recipe in Settings → Project types → Edit → Steps.
- **register-in-registry** — the engine's structural final step (write the registered project row, update FK columns, return the registered project object). It is **not** user-droppable. It is rendered as a non-draggable `[final, automatic] Register in setlist` row at the bottom of every recipe's Steps list. It always runs last in v1; future post-register hooks remain a deferred possibility but are not structurally ruled out.

**Recipes are per-type defaults.** Each project type carries its recipe — the ordered list of primitive invocations with bound parameter values. Recipes are the **default** for the type; they are not a per-call shape today. (Per-call overrides are deferred but not structurally ruled out — recipes attach to project types, not to bootstrap calls, and the data shape leaves room for that future.) The two seeded types (Code project, Non-code project) ship with seeded recipes that match v0.27 behavior: Code project's recipe is `create-folder → copy-template (no-op when template_directory is null) → git-init → update-parent-gitignore`, and Non-code project's recipe is `create-folder → copy-template (no-op when template_directory is null) → update-parent-gitignore`. The register-in-registry trailer is appended automatically.

**Template tokens.** Primitive parameters may reference project context via tokens. The supported set in v1 is:

- `{project.name}` — the project's slug-style identifier (e.g., `space-tracker-v2`)
- `{project.path}` — the project's resolved on-disk path (e.g., `~/Code/space-tracker-v2`)
- `{project.type}` — the resolved project type name (e.g., `Code project`)
- `{project.parent_path}` — the resolved on-disk path of the project's parent (when a parent is declared); pre-flight fails with the standard unresolvable-token shape on parentless projects that reference this token
- `{project.type.template_directory}` — the project type's configured template directory (when set); pre-flight fails when the token is referenced for a type with `template_directory = NULL`
- `{project.email_account}` — the resolved email account for Mail.app integration (added in spec 0.29). Resolution precedence is described under "Email account resolver precedence" below

A literal `{` is escaped as `{{`. Unresolved tokens (e.g., a typo like `{projct.name}`) are pre-flight failures, not run-time crashes. Preview surfaces (see "Dry run and preview" below) substitute placeholder values — `<example-name>`, `<example-path>`, `<example-type>`, `<example-parent-path>`, `<example-template-directory>`, `<example-email-account>` — when no real project exists yet.

**Email account resolver precedence.** When `{project.email_account}` is referenced by any recipe step parameter, the engine resolves the value at pre-flight time using the following precedence, in order:

1. **Project-level `email_account`** — if the project's own `email_account` field is set to a non-null, non-empty string, that value is used.
2. **Recipe-step per-step default** — otherwise, if the recipe step's `params_json` carries a per-step default email account (set in Settings → Project types → Edit → Steps for that primitive invocation), that value is used.
3. **Unresolved** — otherwise, pre-flight fails with the same unresolvable-token shape as any other unresolved token, naming `{project.email_account}` and the step that referenced it.

An empty string is treated as **unresolved** at resolution time (it does not satisfy step 1). NULL is the canonical "unset" state on the project field. The user-facing surface in Settings is "defaults per project type"; the storage location for the per-type default is the recipe step's `params_json` (not a column on `project_types`), because the same `mail-create-mailbox` primitive could appear in multiple steps within one type's recipe with different account targets — binding the default to the recipe step preserves that flexibility and aligns with how every other recipe parameter is bound today. The user does not see the storage detail; from the user's perspective, opening Settings → Project types → <type> → Edit → Steps → mail-create-mailbox surfaces the per-step default as a labelled field.

**Bootstrap engine flow.** When `bootstrap_project` is called with a name, a project type, optionally an area, optionally a parent_project, and optionally a path override:

1. **Snapshot the recipe.** The engine snapshots the project type's current recipe at bootstrap start. Edits the user makes to that type's recipe between failure and Retry do not affect the in-flight Retry — the engine uses the snapshot. New bootstraps started after an edit pick up the new recipe.
2. **Pre-flight.** The engine walks every step in order, resolving template tokens, checking that filesystem-op targets are reachable, that shell-command binaries are on PATH, and that mcp-tool primitives are currently registered with the host MCP session. Pre-flight produces a per-step ✓/✗ trace; any ✗ aborts the bootstrap before any side effect runs. Pre-flight does not execute commands or write files.
3. **Step-by-step execution.** The engine runs each primitive in declared order. Each step's status is recorded (started, succeeded, failed, with timestamps and outcome). On step failure the engine **stops and reports** — see "Failure handling" below.
4. **Register-in-registry trailer.** When all user-droppable steps have succeeded, the engine runs the structural register step: writes the project row, sets `project_type_id`, attaches paths, applies area and parent_project links. The bootstrap response is then assembled and returned.

**Pre-flight and dry run, combined.** Two surfaces expose the per-step trace before any execution touches disk:

- **Bootstrap dialog → `Dry run`.** Run the recipe symbolically against the typed name and chosen type without committing to the bootstrap. The output is a per-step trace showing the resolved operation (e.g., `mkdir -p ~/Code/my-thing`, `cp -r ~/Templates/code-project/* .`, `gh repo create my-thing --private`) and a ✓ or ✗ pre-flight marker for each step. The ✗ rows name what is missing (path doesn't exist, binary not on PATH, MCP tool not registered, unresolved token). No filesystem, network, or external side-effect occurs.
- **Project type editor → `Preview recipe`.** The same per-step trace, but using `<example-name>`, `<example-path>`, and `<example-type>` placeholders so the user can sanity-check a recipe without needing a real project to bootstrap. Pre-flight ✓/✗ checks still run — connectivity and binary presence checks are real even though substitution is symbolic.

**Failure handling: stop and report, resumable.** When a primitive fails mid-run, the engine halts immediately. The user (or calling agent) sees:

- The recipe so far, with each step marked succeeded, failed, or not-yet-run
- The failed step's name, parameters, and error output verbatim
- Three actions: **Retry**, **Skip**, **Abandon**

**Retry** resumes from the failed step against the recipe snapshot. Steps that already succeeded are NOT re-run — including shell-command and mcp-tool steps with external side effects. The engine treats already-succeeded filesystem-op steps idempotently on a fresh recipe walk: if a folder already exists at the target path, copy-template skips files that are already present, etc. (see "Filesystem-op idempotence" in `#rules` (3.3)).

**Skip** marks the failed step as skipped and continues to the next step. The engine does not reason about cross-step dependencies in v1 — Skip continues blindly, and the user owns the consequence. (A future `depends_on` mechanism is deferred.)

**Abandon** ends the bootstrap. The engine **undoes filesystem and git work** it itself performed: deletes the new project folder, removes any `.git` directory it initialized, reverts the parent `.gitignore` append. External side effects from succeeded shell-command and mcp-tool steps are **not** undone — the engine cannot unilaterally delete a Todoist project, undo a `gh repo create`, unsend a webhook, or remove a Mail.app mailbox. The Abandon report explicitly lists, in plain language, what was cleaned up and what was not. **External side effects are reported as a structured `external_side_effects` array, with each entry shaped `{step, primitive, summary}`** — `step` is the 1-based recipe position (matching the pre-flight trace's numbering), `primitive` is the primitive name (e.g., `gh-repo-create`, `mail-create-mailbox`, `create-todoist-project`), and `summary` is a one-line plain-language description naming the externally-held resource (e.g., "GitHub repository `my-thing` still exists", "Mail.app account 'm@h3r3.com' has new mailbox 'Projects/my-thing'", "Todoist project 'My Thing' still exists"). The structured shape lets MCP-tool consumers iterate over the leftover side effects programmatically; the human-readable surface (desktop dialog, CLI output) renders the same array as a "Left in place — clean up manually if needed:" list. The user sees, for example, "Removed `~/Code/my-thing`. Did NOT undo: GitHub repository `my-thing` still exists; Mail.app account 'm@h3r3.com' has new mailbox 'Projects/my-thing'. Clean those up by hand if you want a fully fresh start." Honesty, not silent partial cleanup, is the contract.

**Path override.** The caller can provide an explicit path instead of using the chosen type's default directory. This accommodates projects that need to live in non-standard locations. The override applies to a single bootstrap call; it does not change the stored type configuration.

**When the folder already exists.** If the target folder already exists on disk, the engine's create-folder primitive fails pre-flight (or at run-time, depending on exact race). It does not overwrite, merge into, or silently skip the existing folder. The caller chooses a different name, provides a path override, or handles the existing folder manually before retrying.

**Bootstrap response.** The caller receives confirmation with the registered project name, the chosen type, the created path, and an `executed_steps` array — one entry per recipe step containing `{name, shape, status, error?}`. For backward compatibility with existing callers, the response also carries the legacy boolean flags `git_initialized`, `templates_applied`, and `parent_gitignore_updated` — each set to `true` when the corresponding built-in ran successfully in this recipe, `false` otherwise (including when the user removed it from the recipe). The bootstrap response also includes a `next_steps` array of `{action, why}` entries pointing at the immediate enrichment calls (`enrich_project`, `write_fields`, `register_capabilities`, `refresh_project_digest`) — the same cross-surface pattern described in `#capability-declarations` (2.11).

**Configuration prerequisites.** Bootstrap requires at least one project type to exist. The `archive_path_root` lives in Settings → Bootstrap (see `#desktop-app` (2.14)) for archive cleanup. If `bootstrap_project` is called before any project types exist (a fresh install or a user who has deleted every type), the call fails with: "Bootstrap has no project types configured. Open Settings → Project types to add at least one." The error names the exact next step — no silent fallbacks, no guessing at paths. If a user-authored mcp-tool primitive is referenced in a recipe but the named tool is not registered with the current MCP session, pre-flight fails with a clear message naming the missing tool.

**Archiving with filesystem cleanup.** When a project is archived (via the desktop app, CLI, or MCP), the archive operation can optionally move the project's folder to the configured `archive_path_root`. If an archive path root is configured:
...
- It does not run post-creation hooks or install dependencies as a hidden behavior — anything that happens at bootstrap time is a primitive in the recipe, visible to the user in Settings.
- It does not modify `register_project`. Registration remains pure data with no filesystem side effects. Bootstrap is the higher-level operation that composes registration with folder creation, primitives, and the register-in-registry trailer.
- It does not store, prompt for, or manage credentials of any kind. Auth always lives upstream — in the MCP server for `mcp-tool` primitives, in the user's shell environment for `shell-command` primitives. See `#hard-constraints` (4.3).
- It does not execute user-authored JavaScript or TypeScript. The closed three-shape primitive set is the v1 surface; plugin-style code primitives are explicitly out of scope.
...
- **Register** — A form for creating a new project entry. Collects the six core identity fields (name, display name, type, status, description, goals), optionally filesystem paths, and (added in spec 0.29) optionally `email_account` for Mail.app integration. Calls `registry.registerProject()` through the IPC bridge. The new project appears in the card grid immediately.

- **Edit** — From the project detail header, the user can edit the project's display name, status, description, goals, and (added in spec 0.29) `email_account`. The `email_account` field is a labelled, editable text field placed alongside the other identity fields; saving with a value persists via `update_project`/`write_fields`, and clearing the field (deleting all text and saving) sets the underlying value to NULL — not an empty string. The detail view (read-only) shows the current value when set, and shows nothing or a clear "—" placeholder when null. Validation is presence-only; setlist does not validate the address against Mail.app's account list at edit time. Edits are committed individually or as a batch. Calls `registry.updateProject()` through the IPC bridge.
...
- **Add a type** — opens a form for name, default directory (e.g., `~/Code`, `~/Projects`, `~/Notebooks`), a git-init checkbox, and an optional template directory. The new type ships with an empty recipe (only the register-in-registry trailer); the user composes its steps in the type editor's Steps section described below.
- **Edit a type** — change the name, default directory, git-init flag, template path, or **bootstrap recipe** (see Steps below) inline. When the recipe includes `mail-create-mailbox`, the editor for that recipe step exposes a **per-step default email account** field (the storage location for "per-type default email account" — see `#project-bootstrap` (2.13) "Email account resolver precedence"). The field is labelled, editable, and clearable; setting it persists the value on the recipe step's `params_json`, where the resolver consults it as the second precedence tier after the project's own `email_account` field. Two project types each authored to include `mail-create-mailbox` carry independent per-step defaults — Code's default does not bleed into Non-code's default. Changes apply to subsequent bootstraps; existing projects of that type are unaffected, and an in-flight Retry for a bootstrap that already started uses the recipe snapshot taken at start (see `#project-bootstrap` (2.13)).
...
Two defaults are seeded at install: **Code project** (default directory `~/Code/`, git init on, no template, recipe `create-folder → copy-template → git-init → update-parent-gitignore`) and **Non-code project** (default directory `~/Projects/`, git init off, no template, recipe `create-folder → copy-template → update-parent-gitignore`). Both can be edited or removed once they are no longer attached to any projects.

**Steps subsection (inside the project type editor).** Below the type's name/directory/flags fields, an ordered **Steps** list shows the project type's bootstrap recipe — the primitives that run in order when a project of this type is bootstrapped. Each row in the list is one primitive invocation and shows: a pair of **▲ / ▼ up/down controls** for reordering (the up button on the first row and the down button on the last row are disabled), the primitive's **name**, a **shape badge** (`filesystem-op`, `shell-command`, or `mcp-tool`), a collapsed **parameter summary** (one line, truncated; expandable to inspect or edit bound values), and a **remove** control. Each click on ▲ or ▼ moves the row one position in the corresponding direction. Clicking a row expands it for inline parameter editing — bound values are typed templates that may reference `{project.name}`, `{project.path}`, `{project.type}`. A **`+ Add step`** control at the bottom of the list opens a **primitive picker**: a searchable popover listing every primitive defined in the Primitives panel (built-ins first, then user-authored), filterable by shape, with a one-line description per entry. Selecting a primitive appends it to the recipe with its declared default parameter shape; the user fills in the bound values inline. The register-in-registry step is rendered below the user's list as a non-reorderable, non-removable, **`[final, automatic] Register in setlist`** row — visible so the user sees where it sits in the flow, but unmovable. A **`Preview recipe`** button at the bottom of the editor runs a dry-run pre-flight against `<example-name>` placeholder values and shows the per-step trace described in `#project-bootstrap` (2.13). Empty recipes (only the trailer) are valid: the engine just runs the register step.

**Settings → Primitives.** A peer panel to Project types — a managed list of every bootstrap primitive available to recipes. Two subsections:

- **Built-in primitives** (read-only) — the five primitives setlist ships with: `create-folder`, `copy-template`, `git-init`, `update-parent-gitignore`, and `mail-create-mailbox` (added in spec 0.29). Each row shows the name, shape badge, and a one-sentence description of what it does. They cannot be edited or deleted. They appear in the primitive picker described above and may be dropped into any recipe in any order. The first four are bound to the seeded Code project and Non-code project default recipes out of the box; **`mail-create-mailbox` is seeded but not in any default recipe — users opt in by adding it to a project type's recipe in the type editor's Steps section.**
- **Custom primitives** (user-authored) — a CRUD list of primitives the user has defined. Each row shows the name, shape badge, a parameter summary, a count of recipes that reference it, and edit/delete controls. The user can:
  - **Add a primitive** via a **`+ New primitive`** control that opens a dialog: choose a shape (`filesystem-op`, `shell-command`, `mcp-tool`), give it a name and a one-sentence description, and define its parameter shape. For `filesystem-op`, choose the operation (create-folder, copy, append-to-file) and specify path/content templates. For `shell-command`, paste a verbatim string (multi-line allowed) with `{project.*}` token placeholders; nothing else is rewritten. For `mcp-tool`, pick a tool from the list of currently registered MCP tools in the host session and define its parameter map.
  - **Edit a primitive** — same dialog. Edits flow through to recipes that reference this primitive on the next bootstrap (existing in-flight Retry runs use the snapshot, per `#project-bootstrap` (2.13)).
  - **Delete a primitive** — blocked when any recipe references it. The action surfaces a modal listing the referencing recipes (e.g., "Used by 2 recipes — remove from those first: Code project, Research dossier") and the user must remove the primitive from those recipes before deletion succeeds.
- **Availability badges.** A custom `mcp-tool` primitive whose target tool is not currently registered with the active MCP session shows a warning badge ("Tool not currently registered — bootstraps using this recipe will fail pre-flight"). The primitive itself is preserved on disk; only its run-time availability is affected. See `#rules` (3.3).

**Bootstrap dialog: Dry run.** The bootstrap dialog (the form the user fills to start a new bootstrap from the desktop app) gains a **`Dry run`** button alongside its **`Bootstrap`** button. Pressing Dry run executes the pre-flight + symbolic walk against the typed name and chosen type — same trace shape as the type editor's Preview but using the real values the user has entered. No filesystem or external side effects occur. The output is a per-step list of resolved operations with ✓/✗ markers; rows that ✗ name what is missing. If pre-flight is clean the user can click `Bootstrap` to commit; if not, the user fixes the recipe (or the environment) and re-runs Dry run.
...
**MCP server access.** @setlist/mcp wraps @setlist/core as 56 MCP tools via @modelcontextprotocol/sdk using stdio transport managed by Claude Code's lifecycle. The server provides:
...
- **Projects** -- Initiatives tracked in the registry. They progress through a lifecycle: idea, draft, active, paused, archived, complete. Each project has core identity fields, zero or more extended fields, one or more filesystem paths, an optional area assignment, an optional parent project, and (as of schema v15) an optional `email_account` field for Mail.app integration via the `mail-create-mailbox` bootstrap primitive. Each project also carries a **type** chosen from the user's project types list (see below); the legacy `area_of_focus` project type was retired in schema v11.
...
- **Core identity fields** -- The seven fields every project must have, plus one optional structural relationship and one optional integration field:
  ...
  - **Email account** (optional, added in schema v15) -- the email address of the Mail.app account this project's correspondence belongs under, used by the `mail-create-mailbox` bootstrap primitive to resolve `{project.email_account}` (see `#project-bootstrap` (2.13)). Free-text email-shaped string with presence-only validation — setlist does not validate the address against Mail.app's account list at write time. Settable at registration via `register_project`, editable later via `update_project` and the desktop project detail/edit form. Clearing the field stores NULL, not an empty string; an empty-string value is treated as unresolved at token-resolution time. Per-project values shadow per-type defaults at bootstrap time (see "Email account resolver precedence" in `#project-bootstrap` (2.13)).
...
**Bootstrap primitive and recipe rules:**

- The set of primitive shapes is closed in v1: exactly three values are accepted — `filesystem-op`, `shell-command`, `mcp-tool`. Writes referencing any other shape are rejected with `Error [INVALID_PRIMITIVE_SHAPE]`. There is no `http-call` shape. There is no plugin/code-extension shape. The closed set is the surface that makes the no-arbitrary-code-execution guarantee in `#hard-constraints` (4.3) observable.
- Each primitive has a unique name within the user's primitive set. Built-in primitive names are reserved — users cannot define a custom primitive named `create-folder`, `copy-template`, `git-init`, `update-parent-gitignore`, or `mail-create-mailbox`.
- Built-in primitives are read-only: writes attempting to edit or delete a row with `built_in_flag = true` are rejected with `Error [BUILTIN_PRIMITIVE_IMMUTABLE]`. Their parameter shape, name, and description ship with the binary and may not be modified by the user. The seeded set as of spec 0.29 is five rows: `create-folder`, `copy-template`, `git-init`, `update-parent-gitignore`, `mail-create-mailbox`.
- Deletion of a custom primitive is blocked when any recipe references it. The deletion fails with `Error [PRIMITIVE_HAS_REFERENCES]` and a count of referencing recipes; the Settings panel surfaces the names of those recipes for the user to clean up first. This matches the deletion semantics for areas (`AREA_HAS_PROJECTS`) and project types (`TYPE_HAS_PROJECTS`).
- Recipes are ordered: the `position` column on each recipe row is its zero-based ordinal in the list. Reordering rewrites positions atomically.
- The bootstrap engine executes recipe steps in declared order. There is no implicit reordering, no dependency resolution, no parallelism in v1.
- The engine snapshots the project type's recipe at bootstrap start. Edits to that type's recipe between failure and Retry do not affect the in-flight Retry — the engine uses the snapshot. New bootstraps started after an edit pick up the new recipe.
- Recipe edits do not affect already-bootstrapped projects. Bootstrap is a one-time scaffolding event; recipes govern future bootstraps only.
- Pre-flight runs before any step executes. Pre-flight resolves template tokens, checks filesystem-op target reachability, checks shell-command binary presence on PATH, and checks `mcp-tool` registration with the host MCP session. The `mail-create-mailbox` built-in additionally checks that Mail.app is running via a non-side-effecting process probe (process enumeration via `System Events` or equivalent). Pre-flight does NOT validate that the resolved Mail.app account exists — account-not-found is surfaced as a mid-run failure via the standard Retry/Skip/Abandon dialog. Any pre-flight ✗ aborts the bootstrap before any side effect runs.
- Template tokens supported in v1: `{project.name}`, `{project.path}`, `{project.type}`, `{project.parent_path}`, `{project.type.template_directory}`, and `{project.email_account}`. A literal `{` is escaped as `{{`. Unresolved tokens (typos, unsupported names, missing values) produce a pre-flight ✗ — never a run-time crash. For `{project.email_account}`, resolution follows the precedence project field → recipe-step per-step default → unresolved (see `#project-bootstrap` (2.13)). An empty string on the project's `email_account` field is treated as unresolved, not as a valid value.
- `mail-create-mailbox` is silently idempotent on duplicate mailbox names within the same account: AppleScript's `make new mailbox` returns success without creating a sibling when a mailbox of the same name already exists at the same nested path under the same account. The primitive treats this as a normal success; no special "skipped" or "noop" status is surfaced. Idempotence keys on the resolved nested path, not just the leaf name — a mailbox at `Projects/alpha` does not block creation of a sibling `Code/alpha`.
- Setlist never auto-launches external GUI applications. Specifically, the `mail-create-mailbox` primitive does not invoke `tell application "Mail" to activate`, `open -a Mail.app`, or any other launching mechanism. If Mail.app is not running, pre-flight fails cleanly and the user launches it themselves before re-running.
- On step failure, the engine halts and reports — it does NOT continue past the failure. The user (or calling agent) chooses Retry, Skip, or Abandon.
- **Retry** resumes from the failed step against the recipe snapshot. Steps that already succeeded — including shell-command and mcp-tool steps with external side effects — are NOT re-run.
- **Filesystem-op idempotence on retry.** When a filesystem-op step is reached on a Retry walk and finds its target already in the desired state — the folder already exists for create-folder, the file already exists at the target path for copy-template, the entry is already present in the parent `.gitignore` for update-parent-gitignore — the step succeeds without modification. "Already present" is determined by path presence (file or directory exists at the target); content is not compared.
- `copy-template` does not overwrite existing files. If a file exists at a target path, copy-template skips it. This is what makes Retry safe across recipes that mix create-folder and copy-template.
- **Skip** marks the failed step as skipped and continues. The engine does not reason about cross-step dependencies in v1; Skip continues blindly. (A future `depends_on` mechanism is deferred — not structurally ruled out, but not part of v1.)
- **Abandon** ends the bootstrap. The engine reverses filesystem and git work it itself performed (deletes the new project folder, removes any `.git` directory it initialized, reverts the parent `.gitignore` append). External side effects from succeeded shell-command and mcp-tool steps are NOT undone — the engine has no general inverse for arbitrary commands or MCP tool calls. The Abandon report explicitly enumerates what was cleaned up and what was not.
- `register-in-registry` is the engine's automatic final step. It is not modeled as a primitive, not user-droppable into recipes, not removable, and always runs last in v1. Future post-register hooks remain a deferred possibility but are not structurally ruled out.
- A `shell-command` primitive's command string is treated verbatim — no AST parsing, no shell-injection rewriting, no argument quoting. Only `{project.*}` token substitutions occur. The string is handed to the user's shell exactly as written, multi-line allowed. The exit code determines success/failure: zero is success, non-zero is failure.
- An `mcp-tool` primitive's target tool must be registered with the active MCP session at bootstrap time. Pre-flight ✗ fires when the named tool is absent. The primitive itself is preserved on disk regardless of session-time availability — disabling/re-enabling an MCP server in the host client does not destroy or recreate user-authored mcp-tool primitives.
- The bootstrap engine never holds, prompts for, or stores credentials. Auth lives upstream: in the MCP server for `mcp-tool` primitives, in the user's shell environment for `shell-command` primitives. See `#hard-constraints` (4.3).
...
| Claude Code (via MCP) | All 56 tools | CC reads/writes via @setlist/mcp | CC has no project awareness; MCP server provides it |
...
| Mail.app (macOS, via AppleScript) | Mailbox creation under a user-specified account at a templated path, driven by the `mail-create-mailbox` bootstrap primitive | `shell-command` primitive runs `osascript` against the running Mail.app process to invoke AppleScript's `make new mailbox`; setlist holds no Mail credentials and never auto-launches Mail.app | Pre-flight fails cleanly with "Mail.app must be running to create a mailbox." Account-not-found surfaces mid-run via the standard Retry/Skip/Abandon dialog. |
...
**Bootstrap primitives reach external systems through the user's existing trust boundaries.** The recipe runner introduced in `#project-bootstrap` (2.13) calls out of process in two ways, and in both cases setlist is a passthrough — it never holds credentials of its own:

- **`mcp-tool` primitives delegate to MCP tools registered with the host MCP client** running the bootstrap session (Claude Code, Claude Desktop, Codex, or any other conforming client). Setlist forwards the tool name and parameter map through the host's tool surface and observes the response. Auth, networking, rate limits, and credential lifecycle all live in the MCP server backing that tool — exactly as they do for any other agent invocation. If the user has authorized a Todoist MCP server in their Claude Desktop session, a `Create Todoist project` primitive works during bootstrap; if not, the primitive's pre-flight ✗ surfaces it. Setlist does not store API keys, OAuth tokens, refresh tokens, or service-account credentials for any third party.
- **`shell-command` primitives inherit the user's shell environment** — PATH, environment variables, macOS Keychain access for the user's session, `gh` auth state, `op` 1Password CLI session, AWS profile, kubectl context, and so on. The command runs as the user, in the new project's folder. Setlist does not sandbox, does not strip env vars, does not impose restricted PATH. If `gh` is logged in for the user, `gh repo create` works inside a `shell-command` primitive; if not, the command fails the same way it would in a terminal.
- **`shell-command` primitives can drive Mail.app via AppleScript/`osascript`**, with the running Mail.app session as the trust boundary (added in spec 0.29). The seeded `mail-create-mailbox` built-in is the canonical example: `osascript -e '<applescript>'` invokes AppleScript's `make new mailbox` against the user's already-authenticated Mail.app process. Setlist holds no Mail credentials, no IMAP/SMTP configuration, and no OAuth tokens for any mail provider — the user's Mail.app session is the boundary, exactly as it is when the user adds a mailbox by hand through the Mailbox menu. Setlist does not auto-launch Mail.app; if it is not running, pre-flight fails cleanly and asks the user to start it.

This is intentional: the trust boundary for primitives is the same trust boundary the user already has for their shell, their MCP clients, and their already-authenticated GUI applications. Setlist does not introduce a new credential surface.
...
- Schema v15 (current). v15 adds the `email_account TEXT` column to `projects` to support per-project Mail.app account targeting for the `mail-create-mailbox` bootstrap primitive. Builds on v14's `bootstrap_primitives` and `project_type_recipe_steps` recipe-runner tables, v13's user-managed `project_types` table (replaces the `projects.type` CHECK with a foreign key into it) and reclassified `areas` table (system-owned → user-managed), v12's `project_digests` table, v11's structural `area_id` and `parent_project_id` columns, and v10's unified memory store. Migration history from v8 documented in §5.
...
- Project bootstrap: end-to-end project creation as a **recipe runner**. Each project type carries an ordered recipe of bootstrap **primitives** with bound parameter values; the engine walks the recipe with combined dry-run + pre-flight tracing, stops and reports on mid-run failure with Retry / Skip / Abandon options, and runs the structural register-in-registry step automatically at the end. Three primitive shapes are supported in v1 (`filesystem-op`, `shell-command`, `mcp-tool`); five built-in primitives ship as user-droppable read-only entries (create-folder, copy-template, git-init, update-parent-gitignore, mail-create-mailbox). The `mail-create-mailbox` built-in (added in spec 0.29) drives Mail.app via AppleScript/`osascript` to create a mailbox under a user-specified account at a templated path; seeded but not in any default recipe. Users author additional primitives via Settings → Primitives. Subsumes the manual `new-project.sh` script.
- User-extensible bootstrap: users author custom bootstrap primitives in Settings → Primitives and compose them per-type via the project type editor's Steps section. The user's primitive set may grow without code changes, schema migrations, or re-installs.
...
- Plugin-style code primitives (user-authored JS/TS modules executed by the bootstrap engine). Explicitly out of scope for v1 — the closed three-shape primitive set (`filesystem-op`, `shell-command`, `mcp-tool`) is what makes the no-arbitrary-code-execution stance observable.
- Per-call primitive overrides (passing a one-off primitive list to `bootstrap_project` instead of using the project type's recipe). Recipes attach to types in v1; the data shape leaves room for this future, but it is not part of this spec.
- Cross-step `depends_on` declarations and dep-aware Skip semantics. Skip in v1 continues blindly without reasoning about dependencies.
...
- **Setlist does not store, prompt for, or manage credentials of any kind.** Auth always lives upstream. For `mcp-tool` bootstrap primitives, credentials live in the MCP server backing the named tool — exactly as they do for any other agent invocation of that tool. For `shell-command` bootstrap primitives, credentials live in the user's shell environment (Keychain, `gh` auth state, `op` 1Password session, AWS profile, etc.). Setlist's own database holds no API keys, OAuth tokens, refresh tokens, service-account credentials, or environment-variable copies. There is no credential UI. There is no credential settings panel. There is no plan to add one.

- **`shell-command` primitives run as the user, in the project folder, with the user's shell environment, unsandboxed.** This is a deliberate trust-boundary choice: the user's shell already runs arbitrary commands with the user's privileges, and a `shell-command` primitive is just a recipe-driven invocation of that same surface. Setlist does not impose a restricted PATH, does not strip environment variables, does not run commands in a chroot or container, and does not approve commands per-execution. The user authors the command; the user owns the consequence. Pre-flight binary-presence checks do not constitute approval — they are diagnostic, not gating.

- **No plugin-style code primitives in v1.** The set of primitive shapes is closed: exactly three values are supported (`filesystem-op`, `shell-command`, `mcp-tool`). User-authored JavaScript or TypeScript modules executed by the bootstrap engine are explicitly excluded from v1. This preserves the no-arbitrary-code-execution stance — every recipe step is either a setlist-provided filesystem operation, a user-authored shell command (which already had the same trust profile), or a delegated MCP tool call (whose trust lives in the MCP server). The closed shape set is what makes that stance observable. New shapes may be added in future spec versions if a clear need emerges, but the closed-set invariant survives every such addition.
...
- `schema_meta` — schema version tracking (key PK, value). Stores `schema_version = 15`. Legacy keys related to the retired `area_of_focus` bootstrap type (notably `bootstrap_path_root_area_of_focus`) are removed in the v13 migration.
...
**Schema v14.** v0.28 introduces the user-composable bootstrap primitive system, shipped in v0.5.0. Two new tables back the recipe runner:

- `bootstrap_primitives` — the set of primitives available for composition (id INTEGER PK, name TEXT UNIQUE NOT NULL, shape TEXT NOT NULL CHECK(shape IN ('filesystem-op', 'shell-command', 'mcp-tool')), description TEXT, params_json TEXT NOT NULL, built_in_flag INTEGER NOT NULL DEFAULT 0, created_at TEXT, updated_at TEXT). The `params_json` column carries the shape-specific parameter declarations. Seeded at v13→v14 migration with four built-in rows: `create-folder`, `copy-template`, `git-init`, `update-parent-gitignore`, each with `built_in_flag = 1`. (A fifth built-in row, `mail-create-mailbox`, is added in v15 — see below.) User-authored primitives have `built_in_flag = 0`.
- `project_type_recipe_steps` — the ordered list of primitive invocations attached to each project type (id INTEGER PRIMARY KEY AUTOINCREMENT, project_type_id INTEGER NOT NULL REFERENCES project_types(id) ON DELETE CASCADE, primitive_id INTEGER NOT NULL REFERENCES bootstrap_primitives(id) ON DELETE RESTRICT, position INTEGER NOT NULL, params_json TEXT NOT NULL, UNIQUE(project_type_id, position)). The `ON DELETE RESTRICT` on `primitive_id` enforces the rule that a primitive in use cannot be deleted (`Error [PRIMITIVE_HAS_REFERENCES]`). Seeded at migration with default recipes for the two existing project types matching v0.27 behavior. The `register-in-registry` engine step is NOT stored — it is the engine's automatic trailer.

**Schema v15 (current).** v0.29 adds the `email_account` column to `projects` and seeds the `mail-create-mailbox` built-in primitive:

- `projects.email_account TEXT` — nullable, no default. Used by the `mail-create-mailbox` bootstrap primitive to resolve the `{project.email_account}` token at pre-flight time (see `#project-bootstrap` (2.13) "Email account resolver precedence"). Settable via `register_project`, editable via `update_project` and the desktop project edit form. Clearing the field stores NULL; an empty-string value is treated as unresolved by the token resolver.
- `bootstrap_primitives` row — one new seeded row in v15: `mail-create-mailbox` with `shape = 'shell-command'`, `built_in_flag = 1`, and `params_json` declaring the `account` (string template) and `mailbox_name` (string template, default `Projects/{project.name}`) parameter shape. **No new row is added to any existing project type's `project_type_recipe_steps`** — `mail-create-mailbox` is seeded as a built-in but is not in any default recipe; users opt in by adding it via Settings → Project types → Edit → Steps.

**v14 → v15 migration plan:**

1. `ALTER TABLE projects ADD COLUMN email_account TEXT;` — the column allows NULL and has no default, so every pre-existing project row receives `email_account = NULL` post-migration. No row content is altered beyond the new nullable column.
2. Insert the `mail-create-mailbox` row into `bootstrap_primitives` with `built_in_flag = 1`, the parameter shape declared above, and a one-sentence description (e.g., "Creates a Mail.app mailbox under a specified account at a templated path"). This row is seeded but is not added to any `project_type_recipe_steps` row — default recipes remain unchanged from v14.
3. Bump `schema_meta.schema_version` to 15.
4. The migration is **idempotent**: re-running v15 against an already-v15 database is a no-op (the `ALTER TABLE` is skipped if the column exists; the `INSERT` is skipped if the row exists). Standard migration semantics from prior versions apply: a v15 binary refuses to open a v16+ database (refusing to downgrade is preserved across migrations), and a v14 binary opening a v15 database either fails fast with a clear "schema too new" message or refuses to open — it does not silently downgrade or corrupt.

The migration is non-destructive: no row content is altered, no row is deleted, no other table is touched. The new column is nullable forever — there is no deadline, no migration cutoff, and no future status change forces a value.

The schema sketches above are high-level; the build phase finalizes columns, indexes, and migration steps. The shape `recipe attaches to project_type` does not preclude a future per-call override — recipes are the *default* for the type, not the only allowed shape.
...
interface Project {
  ...
  emailAccount: string | null;         // v15 — Mail.app account for mail-create-mailbox
...
- **Claude Code** — Uses @setlist/mcp for all 56 tools.
...
| server.py | server.ts | 56 MCP tools via @modelcontextprotocol/sdk |
...
- @setlist/mcp exposes 56 MCP tools covering identity, capabilities, memory, ports, tasks, bootstrap, and health, with parameter shapes and response shapes stable across patch releases.
...
**Then layer in:** @setlist/mcp — wrap the core library as 56 MCP tools.
...
- **Bootstrap as a recipe runner with closed primitive shapes.** Project bootstrap is composable per-type rather than fixed-shape, but the set of primitive shapes is closed (`filesystem-op`, `shell-command`, `mcp-tool`). The closed-shape constraint is what makes the no-arbitrary-code-execution stance observable; without it, "user-extensible bootstrap" would have meant "users author plugin code that runs in setlist's process," which is a different system. Auth lives upstream — setlist never holds credentials — which keeps the trust boundary identical to the user's existing shell and MCP client surfaces.
...
| @setlist/mcp | npm package providing the MCP server — 56 tools via @modelcontextprotocol/sdk |
...
| Schema v13 | A prior SQLite schema version, superseded by v14. Added the user-managed `project_types` table, replaced the `projects.type` CHECK constraint with a foreign key into `project_types`, and reclassified the `areas` table from system-owned to user-managed |
| Schema v14 | A prior SQLite schema version, superseded by v15. Adds the `bootstrap_primitives` table (seeded with four built-in rows: create-folder, copy-template, git-init, update-parent-gitignore) and the `project_type_recipe_steps` table (the ordered list of primitive invocations attached to each project type). Backed the user-composable bootstrap recipe runner shipped in v0.5.0 |
| Schema v15 | The current SQLite schema version. Adds the `email_account TEXT` column to the `projects` table (nullable) and seeds a fifth `bootstrap_primitives` row, `mail-create-mailbox` (a `shell-command` shape that drives Mail.app via AppleScript/`osascript`). Backs the per-project Mail.app mailbox creation introduced in spec 0.29; seeded but not in any default recipe |
...
| Project type | A user-managed project kind governing bootstrap behavior and the Type column on Home. Stored in the `project_types` table (seeded at install with two defaults — Code project and Non-code project — and editable via Settings thereafter). Each row carries a default directory, a git-init flag, an optional template directory, and an ordered bootstrap **recipe**. Projects reference a type via `project_type_id`. Renames are label-only; deletion is blocked while projects of that type exist |
| Bootstrap primitive | A named, parameterized step the bootstrap engine knows how to execute. One of three closed shapes in v1: `filesystem-op`, `shell-command`, `mcp-tool`. Five primitives ship as read-only built-ins as of spec 0.29: create-folder, copy-template, git-init, update-parent-gitignore, mail-create-mailbox. Users author additional primitives via Settings → Primitives. Stored in the `bootstrap_primitives` table (introduced in schema v14, extended with the `mail-create-mailbox` row in v15) |
| Recipe | The ordered list of primitive invocations attached to a project type, with each invocation carrying a position and bound parameter values. Recipes are per-type defaults in v1 (per-call overrides are deferred). Snapshotted by the engine at bootstrap start so mid-flight edits don't disrupt in-flight Retry runs. Stored in the `project_type_recipe_steps` table (schema v14). The structural `register-in-registry` engine step is not stored in recipes — it is the engine's automatic trailer |
| Recipe runner | The bootstrap engine's mode of operation in v0.28: walk a snapshotted per-type recipe in order, with combined dry-run + pre-flight tracing, stop and report on mid-run failure with Retry / Skip / Abandon options, run the structural register step automatically at the end |
...
| `email_account` | An optional per-project field (added in schema v15) holding the email address of the Mail.app account this project's correspondence belongs under. Resolved by the `mail-create-mailbox` bootstrap primitive at pre-flight time via the precedence project field → recipe-step per-step default → unresolved. Settable at registration and editable in the desktop project edit form. Clearing the field stores NULL; an empty string is treated as unresolved by the token resolver |
| `mail-create-mailbox` | A seeded built-in bootstrap primitive (added in spec 0.29) of `shell-command` shape that drives Mail.app via AppleScript/`osascript` to create a mailbox under a specified account at a templated path. Idempotent on duplicate names within the same account. Pre-flight checks Mail.app is running but does NOT validate the resolved account exists; account-not-found surfaces mid-run via the standard Retry/Skip/Abandon dialog. Setlist holds no Mail credentials — the user's Mail.app session is the trust boundary. Seeded as a built-in but not added to any project type's default recipe; users opt in by adding the step to a recipe in Settings |
| `external_side_effects` | The structured array on the Abandon report listing externally-held resources that prior succeeded shell-command and mcp-tool steps left in place. Each entry is shaped `{step, primitive, summary}` — `step` is the 1-based recipe position, `primitive` is the primitive name, and `summary` is a one-line plain-language description naming the resource. MCP-tool consumers iterate; humans read the same array as a "Left in place — clean up manually if needed:" list (see `#project-bootstrap` (2.13)) |
| @setlist/app | npm package providing the desktop control panel — Electron app with React renderer |
...
- **Per-call bootstrap primitive overrides.** v0.28's recipe runner attaches recipes to project types; bootstrap calls always execute the type's stored recipe. A future possibility — passing a one-off primitive list to `bootstrap_project` to override the type's recipe for a single call — is not part of this spec but the data shape leaves room for it (recipes are the *default* for a type, not the only allowed shape). Deferred.

- **Cross-step `depends_on` for primitives.** Recipes in v0.28 are flat ordered lists; the engine does not reason about dependencies between steps. A future `depends_on` mechanism would let Skip do dependency-aware fallthrough (e.g., "if create-folder fails, skip everything that needs the folder"). Deferred — Skip continues blindly in v1, and the user owns the consequence.

- **Plugin-style code primitives.** User-authored JavaScript or TypeScript modules executed by the bootstrap engine are explicitly out of scope for v1 (see §4.3 `#hard-constraints`). The closed three-shape primitive set is the v1 surface. Adding a `code` shape later would be a major spec change with new threat-model work.

- **Bootstrap engine post-register hooks.** `register-in-registry` is the structural final step in v1 and always runs last. A future possibility — running additional user-droppable steps *after* registration (e.g., a `notify-team` MCP-tool primitive that fires once the project row is in place) — is not part of this spec but the trailer rendering already signals "final step" rather than "final possible step." Deferred.

- **Additional primitive shapes (`http-call`, `webhook`, etc.).** v0.28 closes the set at three. New shapes may be added in future spec versions if a clear need emerges, but the closed-set invariant survives every such addition. Deferred.

- **Full drag-and-drop reordering for recipe steps.** v0.28 ships ▲/▼ up/down controls on each step row in the project type editor's Steps subsection — one click moves a row one position. A future polish iteration may upgrade this to a true drag-and-drop interaction (drag handle, drop indicator, cross-row animation) once the underlying recipe runner is settled. The data shape — an ordered list with explicit `position` integers — already supports either interaction model. Deferred.
...
Complete tool reference for the 56 MCP tools.
