...
- Behavioral correctness is carried by the 154-scenario holdout set in `.fctry/scenarios.md` (see §4.5 `#testing-discipline`), evaluated by LLM-as-judge; vitest is available for targeted unit tests against @setlist/core where a fast, local signal is useful.
...
The registry can create a new project end-to-end: registering its identity, scaffolding its folder, populating it from templates, optionally initializing version control, and optionally running additional steps the user has composed onto the project's type. Bootstrap is a **recipe runner**: each project type carries an ordered list of bootstrap **primitives** with bound parameters, and the engine walks that recipe in order. This subsumes the manual `new-project.sh` shell script entirely — bootstrap is a first-class registry operation available through MCP, CLI, and library import. Behavioral contract: scenarios S139–S154.
...
- **`filesystem-op`** — Creates folders, copies files or directories, appends to text files. Parameters identify paths and content; the engine resolves `{project.name}`, `{project.path}`, `{project.type}` tokens at run time. The four filesystem-touching built-ins (create-folder, copy-template, update-parent-gitignore — see below) are filesystem-op primitives.
...
**Built-in primitives.** Five steps that previously ran as hardcoded logic inside the engine are recategorized in v0.28:
...
**Template tokens.** Primitive parameters may reference project context via tokens: `{project.name}`, `{project.path}`, `{project.type}`. A literal `{` is escaped as `{{`. Unresolved tokens (e.g., a typo like `{projct.name}`) are pre-flight failures, not run-time crashes. Preview surfaces (see "Dry run and preview" below) substitute placeholder values — `<example-name>`, `<example-path>`, `<example-type>` — when no real project exists yet.
...
**Abandon** ends the bootstrap. The engine **undoes filesystem and git work** it itself performed: deletes the new project folder, removes any `.git` directory it initialized, reverts the parent `.gitignore` append. External side effects from succeeded shell-command and mcp-tool steps are **not** undone — the engine cannot unilaterally delete a Todoist project, undo a `gh repo create`, or unsend a webhook. The Abandon report explicitly lists, in plain language, what was cleaned up and what was not — the user sees, for example, "Removed `~/Code/my-thing`. Did NOT undo: `gh repo create my-thing` (GitHub repository still exists), `Create Todoist project My Thing` (Todoist project still exists). Clean those up by hand if you want a fully fresh start." Honesty, not silent partial cleanup, is the contract.
...
- **Edit a type** — change the name, default directory, git-init flag, template path, or **bootstrap recipe** (see Steps below) inline. Changes apply to subsequent bootstraps; existing projects of that type are unaffected, and an in-flight Retry for a bootstrap that already started uses the recipe snapshot taken at start (see `#project-bootstrap` (2.13)).
...
- **Built-in primitives** (read-only) — the four refactored primitives setlist ships with: `create-folder`, `copy-template`, `git-init`, `update-parent-gitignore`. Each row shows the name, shape badge, and a one-sentence description of what it does. They cannot be edited or deleted. They appear in the primitive picker described above and may be dropped into any recipe in any order.
...
- **Bootstrap primitive** -- A named, parameterized step the bootstrap engine knows how to execute, available for composition into project type recipes. Each primitive has: a **name** (unique within the user's primitive set), a **shape** (one of `filesystem-op`, `shell-command`, `mcp-tool` — the closed set in v1), shape-specific **parameter declarations** (path/content templates for `filesystem-op`, a verbatim command string for `shell-command`, a tool name + parameter map for `mcp-tool`), a one-sentence **description**, and a **built-in flag** distinguishing read-only built-ins from user-authored entries. Four primitives ship as built-ins (`create-folder`, `copy-template`, `git-init`, `update-parent-gitignore`); users may author additional primitives of any of the three shapes via Settings → Primitives. The structural `register-in-registry` engine step is NOT modeled as a primitive — it is the engine's automatic final step and never appears in the primitive set.
...
- Each primitive has a unique name within the user's primitive set. Built-in primitive names are reserved — users cannot define a custom primitive named `create-folder`, `copy-template`, `git-init`, or `update-parent-gitignore`.
- Built-in primitives are read-only: writes attempting to edit or delete a row with `built_in_flag = true` are rejected with `Error [BUILTIN_PRIMITIVE_IMMUTABLE]`. Their parameter shape, name, and description ship with the binary and may not be modified by the user.
...
- Pre-flight runs before any step executes. Pre-flight resolves template tokens, checks filesystem-op target reachability, checks shell-command binary presence on PATH, and checks `mcp-tool` registration with the host MCP session. Any pre-flight ✗ aborts the bootstrap before any side effect runs.
- Template tokens supported in v1: `{project.name}`, `{project.path}`, `{project.type}`. A literal `{` is escaped as `{{`. Unresolved tokens (typos, unsupported names) produce a pre-flight ✗ — never a run-time crash.
...
This is intentional: the trust boundary for primitives is the same trust boundary the user already has for their shell and their MCP clients. Setlist does not introduce a new credential surface.
...
The sketch is high-level; the build phase finalizes columns, indexes, and migration steps. The shape `recipe attaches to project_type` does not preclude a future per-call override — recipes are the *default* for the type, not the only allowed shape. Schema version bumps to 14 when the migration ships.
...
| Schema v14 | The current SQLite schema version. Adds the `bootstrap_primitives` table (seeded with four built-in rows: create-folder, copy-template, git-init, update-parent-gitignore) and the `project_type_recipe_steps` table (the ordered list of primitive invocations attached to each project type). Backs the user-composable bootstrap recipe runner shipped in v0.5.0 |
...
| Bootstrap primitive | A named, parameterized step the bootstrap engine knows how to execute. One of three closed shapes in v1: `filesystem-op`, `shell-command`, `mcp-tool`. Four primitives ship as read-only built-ins (create-folder, copy-template, git-init, update-parent-gitignore); users author additional primitives via Settings → Primitives. Stored in the `bootstrap_primitives` table (schema v14) |
