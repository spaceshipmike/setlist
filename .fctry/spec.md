...
**Steps subsection (inside the project type editor).** Below the type's name/directory/flags fields, an ordered **Steps** list shows the project type's bootstrap recipe — the primitives that run in order when a project of this type is bootstrapped. Each row in the list is one primitive invocation and shows: a **drag handle** for reordering, the primitive's **name**, a **shape badge** (`filesystem-op`, `shell-command`, or `mcp-tool`), a collapsed **parameter summary** (one line, truncated; expandable to inspect or edit bound values), and a **remove** control. Drag-and-drop reorders the row in place. Clicking a row expands it for inline parameter editing — bound values are typed templates that may reference `{project.name}`, `{project.path}`, `{project.type}`. A **`+ Add step`** control at the bottom of the list opens a **primitive picker**: a searchable popover listing every primitive defined in the Primitives panel (built-ins first, then user-authored), filterable by shape, with a one-line description per entry. Selecting a primitive appends it to the recipe with its declared default parameter shape; the user fills in the bound values inline. The register-in-registry step is rendered below the user's list as a non-draggable, non-removable, **`[final, automatic] Register in setlist`** row — visible so the user sees where it sits in the flow, but unmovable. A **`Preview recipe`** button at the bottom of the editor runs a dry-run pre-flight against `<example-name>` placeholder values and shows the per-step trace described in `#project-bootstrap` (2.13). Empty recipes (only the trailer) are valid: the engine just runs the register step.
...
- Schema v13 (current); schema v14 planned with the build of this spec. v13 adds the user-managed `project_types` table, replaces the `projects.type` CHECK with a foreign key into it, and reclassifies the `areas` table from system-owned to user-managed. Builds on v12's `project_digests` table, v11's structural `area_id` and `parent_project_id` columns, and v10's unified memory store. v14 (planned) adds `bootstrap_primitives` and `project_type_recipes` to back the user-composable bootstrap recipe runner. Migration history from v8 documented in §5.
...
**Schema v14 (planned with /fctry:execute on this spec).** v0.28 introduces the user-composable bootstrap primitive system. Two new tables back the recipe runner; the actual migration ships when this spec is built:
...
- `project_type_recipes` — the ordered list of primitive invocations attached to each project type (project_type_id INTEGER NOT NULL REFERENCES project_types(id) ON DELETE CASCADE, primitive_id INTEGER NOT NULL REFERENCES bootstrap_primitives(id) ON DELETE RESTRICT, position INTEGER NOT NULL, bound_params_json TEXT NOT NULL, PRIMARY KEY (project_type_id, position)). The `ON DELETE RESTRICT` on `primitive_id` enforces the rule that a primitive in use cannot be deleted (`Error [PRIMITIVE_HAS_REFERENCES]`). Seeded at migration with default recipes for the two existing project types matching v0.27 behavior. The `register-in-registry` engine step is NOT stored — it is the engine's automatic trailer.
...
| Bootstrap primitive | A named, parameterized step the bootstrap engine knows how to execute. One of three closed shapes in v1: `filesystem-op`, `shell-command`, `mcp-tool`. Four primitives ship as read-only built-ins (create-folder, copy-template, git-init, update-parent-gitignore); users author additional primitives via Settings → Primitives. Stored in the `bootstrap_primitives` table (planned for schema v14) |
| Recipe | The ordered list of primitive invocations attached to a project type, with each invocation carrying a position and bound parameter values. Recipes are per-type defaults in v1 (per-call overrides are deferred). Snapshotted by the engine at bootstrap start so mid-flight edits don't disrupt in-flight Retry runs. Stored in the `project_type_recipes` table (planned for schema v14). The structural `register-in-registry` engine step is not stored in recipes — it is the engine's automatic trailer |
