...
export const MCP_TOOL_DEFINITIONS: McpToolDefinition[] = [
  ...
  { name: 'list_projects', description: 'List all projects at a given depth, with optional type/status/area filters. Suggestion: use get_project() for full details on any project.', inputSchema: { type: 'object' as const, properties: { detail: { type: 'string', enum: ['minimal', 'summary', 'standard', 'full'], default: 'summary' }, type_filter: { type: 'string' }, status_filter: { type: 'string' }, area_filter: { type: 'string', description: 'Canonical area name (Work/Family/Home/Health/Finance/Personal/Infrastructure) or "__unassigned__" for projects with no area' } } } },
      ...
      { name: 'register_project', description: 'Register a new project in the registry. Optional area assigns to one of the 7 canonical areas; optional parent_project links as a sub-project; optional email_account drives the mail-create-mailbox bootstrap primitive (spec 0.29). Suggestion: use update_project() to modify existing projects.', inputSchema: { type: 'object' as const, properties: { name: { type: 'string' }, display_name: { type: 'string' }, project_type: { type: 'string', enum: ['project'], default: 'project' }, status: { type: 'string', default: 'active' }, description: { type: 'string' }, goals: { type: 'array', items: { type: 'string' }, description: 'List of goal statements (one per array element). Legacy string input is accepted but arrays are canonical.' }, paths: { type: 'string' }, area: { type: 'string', description: 'Canonical area: Work, Family, Home, Health, Finance, Personal, or Infrastructure' }, parent_project: { type: 'string', description: 'Name of the parent project for sub-project linking' }, email_account: { type: ['string', 'null'], description: 'Optional email address that drives the mail-create-mailbox bootstrap primitive\'s {project.email_account} token. Free-text, presence-only validation, NULL or empty string both clear.' }, producer: { type: 'string', default: 'system' } }, required: ['name'] } },
      { name: 'update_project', description: 'Update core identity fields on an existing project, including optional area, parent_project, and email_account. Pass null to clear area, parent_project, or email_account.', inputSchema: { type: 'object' as const, properties: { name: { type: 'string' }, display_name: { type: 'string' }, status: { type: 'string' }, description: { type: 'string' }, goals: { type: 'array', items: { type: 'string' }, description: 'List of goal statements (one per array element).' }, area: { type: ['string', 'null'] }, parent_project: { type: ['string', 'null'] }, email_account: { type: ['string', 'null'], description: 'Spec 0.29: optional email address driving mail-create-mailbox. Pass null or "" to clear.' } }, required: ['name'] } },
      ...
      { name: 'portfolio_brief', description: 'Structured portfolio snapshot: active projects, portfolio memories, health indicators, pending observations, and enrichment_gaps annotations flagging registered projects missing description / tech_stack / digest. Use at session start for portfolio-level reasoning.', inputSchema: { type: 'object' as const, properties: {} } },
      // Memory Admin (5)
      ...
      switch (name) {
        ...
        case 'register_project': {
          ...
          registry.register({
            ...
            email_account: a.email_account === null ? null : (a.email_account as string | undefined),
          });
          result = {
        ...
        case 'update_project':
          registry.updateCore(a.name as string, {
            ...
            // spec 0.29: allow updating email_account (null or "" clears)
            email_account: (a.email_account === null ? null : a.email_account as string | undefined),
          });
        ...
        case 'bootstrap_project': {
          ...
          if (useRecipePath || dryRun) {
            const env = await bootstrapManager.bootstrapWithRecipe({
              ...
              // Spec 0.29: drive {project.email_account} for the mail-create-mailbox primitive.
              email_account: a.email_account === null ? null : (a.email_account as string | undefined),
              dry_run: dryRun,
            });
          ...
          const bootstrapResult = bootstrapManager.bootstrapProject({
            ...
            area: a.area as string | undefined,
            parent_project: a.parent_project as string | undefined,
            // Spec 0.29: persist email_account on legacy-path bootstrap too.
            email_account: a.email_account === null ? null : (a.email_account as string | undefined),
          });
          result = {
            ...bootstrapResult,
